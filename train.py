...
def run_experiment_6_localization_seed(df, output_dir):
    """
    Corre el experimento 6 (SEED) y guarda:
    1. El mejor modelo DE CADA CORRIDA (para Media±DE).
    2. El MEJOR MODELO GLOBAL de todas las corridas (para uso antiguo).
    """
    print("\n--- INICIANDO EXPERIMENTO 6: LOCALIZACIÓN (SEED) ---")
    
    # --- Preparación de Datos ---
    # (Tu código de dataloaders no cambia)
    df_localizar = df[['signal', 'labels', 'existeK']]
    train_df, temp_df = train_test_split(
        df_localizar, test_size=0.2, random_state=42, stratify=df_localizar['existeK']
    )
    ...
    # --- ¡LÓGICA DE GUARDADO DOBLE! ---
    
    # 1. Path para el MEJOR MODELO GLOBAL (para funciones antiguas)
    global_best_model_path = os.path.join(output_dir, 'best_model_localization_seed.pth')
    global_best_val_loss = float('inf')
    global_best_epoch = 0 
    ...
    for i in range(NUM_RUNS):
        ...
        # 2. Path para el MEJOR DE ESTA CORRIDA (para Media±DE)
        current_run_save_path = os.path.join(output_dir, f'best_model_localization_seed_run{i+1}.pth')
        ...
        for epoch in range(EPOCHS):
            ...
            if val_loss < current_run_best_val_loss:
                ...
                # 1. Guardar el mejor de la corrida actual
                torch.save(model.state_dict(), current_run_save_path)
                print(f"  -> Nuevo mejor modelo para Run {i+1} guardado (Época {epoch+1})")

                # --- ¡LÓGICA GLOBAL AÑADIDA! ---
                # 2. Comprobar si también es el MEJOR GLOBAL
                if val_loss < global_best_val_loss:
                    global_best_val_loss = val_loss
                    global_best_epoch = epoch + 1
                    # Guardar en el path del modelo global
                    torch.save(model.state_dict(), global_best_model_path)
                    print(f"  -> *** ¡NUEVO MEJOR MODELO GLOBAL GUARDADO! (Loss: {global_best_val_loss:.4f}) ***")
                # --- FIN LÓGICA GLOBAL ---
        ...
        all_histories.append(history)
        print(f"Mejor modelo para Run {i+1} guardado en: {current_run_save_path}")
    ...
    plot_avg_training_history(
        all_histories, 
        title_suffix='(SEED Localización)', 
        best_epoch=global_best_epoch, # (Ahora esto vuelve a ser útil)
        save_path=os.path.join(output_dir, 'training_curves_exp6_seed.png') 
    )
    
    print("--- FIN EXPERIMENTO 6 ---")
